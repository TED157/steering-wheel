...
fp32 wz;
fp32 roting_speed;
fp32 Angle_zero_6020[4] = {-78.9, -57.6, -130.7, 33.3};
...
void ChassisCommandUpdate()
{
	...
	if (Chassis.Mode == FALLOW || Chassis.Mode == ROTING || Chassis.Mode == STOP ) //
	{
		...
		if (Chassis.Mode == FALLOW)
		{
			...
			if((Fabs(Chassis.vx)<0.5 && Fabs(Chassis.vy)<0.5)&&Fabs(angle_minus)<13&&Chassis.vx*Chassis.vy!=0) 
				Chassis.wz*=0.01;
			//Chassis.wz = 0;
		}
		else if (Chassis.Mode == ROTING)
		{
			angle_minus = -YawMotorMeasure.angle + FollowAngle - YawMotorMeasure.speed_rpm * 0.69;
			Chassis.wz = sin(v_gain/4.2)*3.0;
			Chassis.vx = ((PTZ.FBSpeed / 32767.0f) * cos(angle_minus/180.0*PI) - (PTZ.LRSpeed / 32767.0f) * sin(angle_minus/180.0*PI))*v_gain/2.1;//* (1.0f + Chassis.Power_Proportion /Power_Max );
			Chassis.vy = ((PTZ.FBSpeed / 32767.0f) * sin(angle_minus/180.0*PI) + (PTZ.LRSpeed / 32767.0f) * cos(angle_minus/180.0*PI))*v_gain/2.1;//* (1.0f + Chassis.Power_Proportion /Power_Max );
		}
...
void RefereeInfUpdate(ext_game_robot_status_t *referee)
{
	...
	switch(referee->chassis_power_limit)
	{
		...
		case 70:	
			Power_Max = 70;kp=1.19 * 1.99999999e-06;v_gain=2.5;break;
		case 75:	
			Power_Max = 75;kp=1.19 * 1.99999999e-06;v_gain=2.7;break;
		case 80:
			Power_Max = 80;kp=1.19 * 1.99999999e-06;v_gain=3.0;break;
		case 85:	
